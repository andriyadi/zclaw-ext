...
static int32_t cores3_main_app_init_hook(axp2101_t *pmic, void *user_ctx) {
  custom_cores3_app_context_t *app_ctx =
      (custom_cores3_app_context_t *)user_ctx;
  if (pmic == NULL || app_ctx == NULL) {
    ESP_LOGE(TAG, "pmic == NULL ? %d app_ctx == NULL ? %d", pmic == NULL,
             app_ctx == NULL);
...
static void clear_boot_count(void *arg) {
  ...
  if (boot_count_err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to clear boot counter: %s",
...
static bool check_factory_reset(void) {
  if (!gpio_policy_runtime_input_pin_is_safe(FACTORY_RESET_PIN)) {
    ESP_LOGW(
        TAG,
        "Skipping factory reset button check: pin %d is unsafe on this target",
        FACTORY_RESET_PIN);
    return false;
  }
...
void app_main(void) {
  ...
  hooks->user_ctx = &s_cores3_main_app_ctx;
  hooks->tick_1s_hook = custom_cores3_app_refresh;
  hooks->init_hook = cores3_main_app_init_hook;
  hooks->reboot_hook = cores3_main_app_reboot_hook;

  xTaskCreate(cores3_app_task, "cores3_app", CORES3_APP_TASK_STACK_SIZE_DEFAULT,
  ...
  if (boot_count_err != ESP_OK) {
    ESP_LOGW(TAG, "Failed to persist boot counter: %s",
             esp_err_to_name(boot_count_err));
  }
  ...
  QueueHandle_t input_queue =
      xQueueCreate(INPUT_QUEUE_LENGTH, sizeof(channel_msg_t));
  QueueHandle_t channel_output_queue =
      xQueueCreate(OUTPUT_QUEUE_LENGTH, sizeof(channel_output_msg_t));
  ...
  // 8. Register tools and local channel early so /gpio and /diag work before
  // WiFi.
  tools_init();
  channel_init();
  ...
  bool telegram_enabled =
      device_configured && !s_safe_mode && telegram_is_configured();
#endif
  if (telegram_enabled) {
    telegram_output_queue =
        xQueueCreate(TELEGRAM_OUTPUT_QUEUE_LENGTH, sizeof(telegram_msg_t));
  }
  ...
  if (!device_configured || s_safe_mode) {
    if (s_safe_mode) {
      ...
      channel_write(
          ...
          "Try /gpio, /diag, /reboot, /wifi, /bootcount, /clear-safe-mode, "
          "/factory-reset, /help, or /settings.\r\n\r\n");
    } else {
      print_provisioning_help();
      channel_write("\r\nDevice is not provisioned.\r\n"
                    "Local serial commands remain available: /gpio, /diag, "
                    "/reboot, /wifi, /bootcount, /clear-safe-mode, "
                    "/factory-reset, /help, /settings.\r\n\r\n");
    }
    while (1) {
  ...
  if (xTaskCreate(clear_boot_count, "boot_ok", BOOT_OK_TASK_STACK_SIZE, NULL, 1,
                  NULL) != pdPASS) {
